...
def atom37_to_torsions(all_atom_positions, aatype, all_atom_mask=None):
    ...
    if type(aatype) is np.ndarray:
        aatype = torch.from_numpy(aatype)
    if all_atom_mask is None:  # shape [bs, L, 37]
    ...
    pre_omega_atom_pos = torch.cat(  # points to CA(i-1), C(i-1), N(i), CA(i), shape [bs, L, 4, 3]
        [prev_all_atom_positions[..., 1:3, :], all_atom_positions[..., :2, :]],
        dim=-2,
    )
    phi_atom_pos = torch.cat(  # points to C(i-1), N(i), CA(i), C(i), shape [bs, L, 4, 3]
        [prev_all_atom_positions[..., 2:3, :], all_atom_positions[..., :3, :]],
        dim=-2,
    )
    psi_atom_pos = torch.cat(  # points to N(i), CA(i), C(i), O(i), shape [bs, L, 4, 3]
        [all_atom_positions[..., :3, :], all_atom_positions[..., 4:5, :]],
        dim=-2,
    )

    pre_omega_mask = torch.prod(prev_all_atom_mask[..., 1:3], dim=-1) * torch.prod(  # shape [bs, L]
        all_atom_mask[..., :2], dim=-1
    )
    phi_mask = prev_all_atom_mask[..., 2] * torch.prod(  # shape [bs, L]    
        all_atom_mask[..., :3], dim=-1, dtype=all_atom_mask.dtype
    )
    psi_mask = (  # shape [bs, L]
        torch.prod(all_atom_mask[..., :3], dim=-1, dtype=all_atom_mask.dtype)
        * all_atom_mask[..., 4]  # shape [bs, L]
    )
    ...
    torsion_angles_mask = torch.cat(  # shape [bs, L, 7]
        [
            pre_omega_mask[..., None],
            phi_mask[..., None],
            psi_mask[..., None],
    ...
    return torsion_angles_sin_cos, torsion_angles_mask  # shape [bs, L, 7, 2], [bs, L, 7]
...
def atom14_to_frames(atom14):
    ...
    rots[:,:, 0, 0] = -1
    rots[:,:, 2, 2] = -1  # a reflection across the y-axis (or equivalently flipping x and z).
    rots = Rotation(rot_mats=rots)
    return prot_frames.compose(Rigid(rots, None))
