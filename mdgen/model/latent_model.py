...
from .window_mha import MultiheadAttention as WindowMHT
import numpy as np
...
class LatentMDGenModel(nn.Module):
    def __init__(self, args, latent_dim, stage='fine'):
        super().__init__()
        self.args = args
        if self.args.design:
        ...
        if self.args.model=='fractal_latent_md_gen' and self.args.abs_time_emb and stage=='coarse': cond_dim -= 8 * self.args.coarse_stride
        if self.args.model=='fractal_latent_md_gen' and self.args.abs_time_emb and stage=='fine':
            cond_dim -= 8
            self.latent_to_emb_coarse = nn.Linear(cond_dim*args.coarse_stride, args.embed_dim)
        if self.args.model == 'fractal_latent_md_gen_3l' and self.args.abs_time_emb:
            cond_dim -= 8 * getattr(self.args, 'cat_factor', 1)
        if self.args.model == 'fractal_latent_md_gen_3l' and getattr(self.args, 'coarse_in_embed_space', False):
            self.latent_to_emb_coarse = nn.Linear(args.embed_dim, args.embed_dim)
        ...
        self.layers = nn.ModuleList(
            [
                LatentMDGenLayer(
                    ...
                    use_window_attention=args.use_window_attention,
                )
                for _ in range(args.num_layers)
            ]
        )
        ...
        if not (self.args.dynamic_mpnn or self.args.mpnn):
            self.emb_to_latent = FinalLayer(args.embed_dim, cond_dim)
        if args.design:
        ...
        if args.abs_time_emb:
            if args.model!='fractal_latent_md_gen':
    ...
    def initialize_weights(self):
        ...
        if self.args.abs_time_emb:
            if self.args.model!='fractal_latent_md_gen':
    ...
    def padding(self, x, mask):
        ## [B, T//stride, L, embed_dim] -> [B, T//stride + 2*pad_size, L, embed_dim] -> [B*T//stride, 2*pad_size, L, embed_dim]
        T_stride = x.shape[1]
        pad_size = self.args.coarse_stride // 2
        # import pdb; pdb.set_trace()
        # Create padding tensors on the same device as x
        front_pad = torch.zeros(x.shape[0], pad_size, x.shape[2], x.shape[3], device=x.device)
        back_pad = torch.zeros(x.shape[0], pad_size, x.shape[2], x.shape[3], device=x.device)
        x = torch.cat([front_pad, x, back_pad], dim=1)
        
        # Extract sliding windows using list comprehension
        windows = [x[:, i:i+T_stride].unsqueeze(2) for i in range(2*pad_size)]
        x = torch.cat(windows, dim=2)
        x = x.reshape(x.shape[0]*T_stride, 2*pad_size, x.shape[3], x.shape[4])
        mask = torch.ones(x.shape[0], 2*pad_size, x.shape[2], device=x.device)  # TODO: need to revise this to fit more tasks like in-painting, etc..
        return x, mask

    def forward(self, x, t, mask,
                start_frames=None, end_frames=None,
                x_cond=None, x_cond_mask=None,
                aatype=None, x_coarse=None, coarse_mask=None
                ):
        """
        This is the forward pass of the LatentMDGenModel.
        Input: 
            x: [bsz, #frms, #residues, latent_dim]  # latent_dim here is either 21 or 29, which represents offsets + c.v. + type
            t: [bsz]  # time steps for the trajectory
            mask: [bsz, #frms, #residues]  # mask for the trajectory
            start_frames [bsz, #residues]:  # start frames of the trajectory
            end_frames [bsz, #residues]:  # end frames of the trajectory
            x_cond: [bsz, #frms, #residues, latent_dim]  # conditioning tokens, latent_dim here is either 21 or 29, which represents offsets + c.v. + type
            x_cond_mask: [bsz, #frms, #residues]  # mask for the conditioning tokens
            aatype: [bsz, #residues]  # amino acid types
        Output:
            latent: [bsz, #frms, #residues, latent_dim]  # latent_dim here is either 21 or 29, which represents offsets + c.v. + type
        """
        ...
        if x_coarse is not None:
            # import pdb; pdb.set_trace()
            x_coarse = self.latent_to_emb_coarse(x_coarse)  # [B, T//stride, L, stride*latent_dim] -> [B, T//stride, L, embed_dim]  
            x_coarse, mask_coarse = self.padding(x_coarse, coarse_mask)  # [B*T//stride, 2*pad_size, L, embed_dim]
            x = torch.cat([x, x_coarse], dim=1)  # [B*T//stride, 2*pad_size + T//stride, L, embed_dim]
            mask = torch.cat([mask, mask_coarse], dim=1)  # [B*T//stride, 2*pad_size + T//stride, L]
        
        t = self.t_embedder(t * self.args.time_multiplier)[:, None]
        if self.args.prepend_ipa:  # IPA doesn't need checkpointing
            x = x + self.run_ipa(t[:, 0], mask[:, 0], start_frames, end_frames, aatype, x_d=x_d)[:, None]  # conditioned on the first frame
...
class AttentionWithRoPE(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        self.need_weights = kwargs.get('need_weights', False)

    def forward(self, x, mask):
        x = x.transpose(0, 1)
        x, weights = self.attn(query=x, key=x, value=x, key_padding_mask=1 - mask, need_weights=self.need_weights)
...
class WindowAttention(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.attn = WindowMHT(*args, **kwargs)
        self.need_weights = kwargs.get('need_weights', False)
...
class LatentMDGenLayer(nn.Module):
    ...
    def __init__(self, embed_dim, ffn_embed_dim, mha_heads, dropout=0.0, num_frames=50, hyena=False,
                 use_rotary_embeddings=False, use_time_attention=True, ipa_args=None, use_window_attention=False):
        super().__init__()
        self.embed_dim = embed_dim
        ...
        self.use_rotary_embeddings = use_rotary_embeddings  
        self._init_submodules(add_bias_kv=True, dropout=dropout, ipa_args=ipa_args, use_window_attention=use_window_attention)

    def _init_submodules(self, add_bias_kv=False, dropout=0.0, ipa_args=None, use_window_attention=False):
