...
class Transport:
    ...
    def training_losses(
            ...
            mask=None,
            model_kwargs=None,
            beta=0.5,  # accepted for API compatibility with the wrapper; unused for 3L (single-velocity loss).
    ):
        ...
        t, x0, x1 = self.sample(x1)  # sample x0 & t based on shape of x1
        t, xt, ut = self.path_sampler.plan(t, x0, x1)  # compute path probability density and vector field
        ...
        if isinstance(out, tuple):
            model_output, v1, v2 = out
        else:
            model_output, v1, v2 = out, None, None
        B, *_, C = xt.shape
        ...
        if not (self.args.dynamic_mpnn or self.args.mpnn):
            if self.model_type == ModelType.VELOCITY:
                terms['loss_fine'] = mean_flat(((model_output - ut) ** 2), mask)
                terms['loss'] = terms['loss_fine']
                if v1 is not None and v2 is not None:
                    terms['loss_l1'] = mean_flat(((v1 - ut) ** 2), mask)
                    terms['loss_l2'] = mean_flat(((v2 - ut) ** 2), mask)
                    terms['loss'] = terms['loss_fine'] + \
                        self.args.aux_loss_weight * 0.5 * (terms['loss_l1'] + terms['loss_l2'])
            else:
